...
def generate_dsa_report():
    root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    
    dsa_content = """# 🧬 BÁO CÁO CHUYÊN SÂU: PHÂN TÍCH CẤU TRÚC DỮ LIỆU & GIẢI THUẬT (DSA AUDIT) TRONG BACKEND EDUCARELINK
...
| **5. Trừ & gộp khoảng thời gian rảnh** | Interval Merging (Sweep-line) | Duyệt lồng nhau $O(C \times B)$ + Sort Merging | 🟡 **Chấp nhận được với tập nhỏ** |
...
* **Độ phức tạp:**
  ...
  * Thêm mới (Insert): $O(1)$
  * Xóa bỏ (Evict): $O(1)$
  * Không gian bộ nhớ: $O(\text{Capacity})$
...
* **Cấu trúc dữ liệu tối ưu thay thế:** **Min-Heap (Đống cực tiểu có kích thước $K$)**:
  ...
  * Độ phức tạp thời gian giảm từ $O(N \log N)$ xuống **$O(N \log 8) \approx O(3N)$ (Độ phức tạp tuyến tính $O(N)$)**!
...
* **Phân tích nhược điểm DSA:**
  ...
  * Mỗi khi có một vết cắt (Cut - là lịch bận hoặc booking đang chạy), hàm duyệt qua toàn bộ các khoảng còn lại. Độ phức tạp là $O(C \times B)$ với $C$ là số vết cắt và $B$ là số khoảng rảnh cơ sở.
...
* **Cấu trúc dữ liệu tối ưu lý thuyết:**
  * **Interval Tree (Cây khoảng thời gian):** Dựa trên cây đỏ-đen (Red-Black Tree), cho phép tìm kiếm tất cả các khoảng thời gian giao nhau với một khoảng $[t_1, t_2]$ bất kỳ trong thời gian $O(\log M + K)$.
...
* **Độ phức tạp:**
  * Mã hóa tọa độ thành GeoHash: $O(\text{Precision})$ — cực nhanh.
...
* **Nguyên lý giải thuật:**
  * Nhằm tránh hiện tượng "chuông báo động rung lắc liên tục" khi CarePartner đứng mấp mé ở ranh giới bán kính an toàn ($R$), giải thuật áp dụng chu trình trễ với 2 ngưỡng:
    1. **Ngưỡng kích hoạt cảnh báo sớm:** $D \ge 0.8 \times R$ ➔ Bắn cảnh báo đón đầu đúng 1 lần, set cờ `predictive_warned = True`.
    2. **Ngưỡng xóa cờ phục hồi:** Chỉ xóa cờ khi $D < 0.8 \times R$ (đã đi lùi hẳn vào sâu bên trong an toàn).
...
* **Vấn đề thực tế:** Tín hiệu GPS di động luôn có sai số từ $15\text{m} - 30\text{m}$ do phản xạ nhà cao tầng hoặc thời tiết. Hiện tại hệ thống đang lấy tọa độ thô để tính toán cự ly.
* **Giải thuật cần bổ sung:** **1D/2D Discrete Kalman Filter**:
  * Dự đoán vị trí tiếp theo dựa trên vận tốc và hướng di chuyển ($v, \text{heading}$).
...
class SimpleKalmanFilter1D:
    """Bộ lọc Kalman 1D tối ưu khử nhiễu trôi GPS."""
...
"""
    ...
    out_docs = os.path.join(root_dir, 'docs', 'BAO_CAO_CHUYEN_SAU_DSA_DATA_STRUCTURES_ALGORITHMS.md')
    ...
    if os.path.exists(artifact_dir):
        out_artifact = os.path.join(artifact_dir, 'BAO_CAO_CHUYEN_SAU_DSA_DATA_STRUCTURES_ALGORITHMS.md')
