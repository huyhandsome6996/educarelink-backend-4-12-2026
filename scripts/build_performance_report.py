...
def generate_report():
    ...
    report_content = """# 📊 BÁO CÁO TOÀN DIỆN: ĐÁNH GIÁ THUẬT TOÁN, TỐC ĐỘ VÀ HIỆU NĂNG HỆ THỐNG BACKEND EDUCARELINK
...
2. **Soft Scoring (Chấm điểm mềm đa tiêu chí):**
   ...
   * Công thức tổng hợp:
     $$\text{Final} = \min\left(100, \max\left(0, \text{round}\left( \frac{\sum w_i s_i}{100} \times \text{BandMultiplier} \right)\right)\right)$$
...
* Điểm ELO hiệu dụng:
  $$\text{Effective ELO} = \text{Base} + \sum \text{Rewards} + \sum \left( \text{Penalty}_i \times \text{DecayFactor}(t_i) \right)$$
...
"""
    ...
    # 1. Ghi vào thư mục docs
    out_docs = os.path.join(root_dir, 'docs', 'BAO_CAO_THUAT_TOAN_HIEU_NANG_HE_THONG.md')
    with open(out_docs, 'w', encoding='utf-8') as f:
        f.write(report_content)
    print(f"Written to: {out_docs}")

    # 2. Ghi vào Brain Artifact
    artifact_dir = r"C:\Users\PC\.gemini\antigravity\brain\89abb596-b339-425c-9480-94fe8f910762"
    if os.path.exists(artifact_dir):
        out_artifact = os.path.join(artifact_dir, 'BAO_CAO_THUAT_TOAN_HIEU_NANG_HE_THONG.md')
