...
class Command(BaseCommand):
    ...
    def _seed_notification_templates(self):
        tpls = [
            ...
            ('booking_committed_parent', 'important', 'parent', 'CarePartner đã xác nhận đơn',
             '{name} đã cam kết nhận công việc — sẽ có mặt lúc {time} ngày {date}.'),
            ('commit_expired', 'critical', 'parent', 'CarePartner không phản hồi đúng hạn',
             'CarePartner {name} không xác nhận trong thời hạn cam kết. Đơn đã hết hạn — hệ thống đang đề xuất người thay thế.'),
