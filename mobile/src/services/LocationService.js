...
// ====================================================================
// MERGE (merge/safety-module-into-main-20260816):
// - P0 FIX (v1.1.5 from main): Bọc try/catch quanh TaskManager.defineTask
//   ở top-level. File này được import TĨNH qua chuỗi App.js → AppNavigator
//   → MyJobsScreen → LocationService, nên code top-level chạy ngay khi
//   Metro resolve dependency graph — TRƯỚC KHI React render. Nếu defineTask
//   throw (native module linking lỗi trong release build), ErrorBoundary
//   (component) KHÔNG bắt được → app crash-on-launch, màn đen.
// - Phần 1 (from feature): nếu apiClient.post fail do mạng → enqueue vào
//   SQLite queue (OfflineLocationQueue) thay vì chỉ log warning. Queue sẽ
//   flush khi có mạng lại.
// - QA-FIX-2 / B2 (from feature): enqueueLocation yêu cầu userId — đọc từ
//   storage 'user_id' (set bởi AuthContext.login). Nếu không có userId →
//   skip enqueue (không cho phép queue vô danh).
  ...
  TaskManager.defineTask(HEARTBEAT_TASK_NAME, async ({ data, error }) => {
    ...
    try {
      ...
      await apiClient.post('/tracking/heartbeat/', {
        task_id: parseInt(taskId, 10),
        // QA-FIX-2 / E (feature): dùng ?? null thay vì || null — tọa độ 0 hợp lệ
...
function startForegroundLocationInterval(taskId, userId) {
...
async function sendCurrentLocation(taskId, userId) {
  try {
